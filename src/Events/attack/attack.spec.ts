...
feature(featureEventDescription(Action.attack), () => {
    serverScenario(`${Action.attack} 1 - Attack Game Event - playerA Tower`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(position(2, 2), ShapeType.robot).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(2, 2), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(3, 2), ShapeType.tower).save()
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(3, 2), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower))
        ])
    serverScenario(`${Action.attack} 2 - Attack Game Event - playerB Tower`, attackEvent(EntityId.playerB, EntityId.playerBTower, EntityId.playerATower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(2, 2), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(3, 2), ShapeType.tower).save()
            .buildEntity(EntityId.playerBRobot).withPhysicalComponent(position(4, 2), ShapeType.robot).save()
        ...
        , [
            ...
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerBTower, EntityId.playerATower))
        ])
    serverScenario(`${Action.attack} 3 - Attack Game Event - playerA Robot`, attackEvent(EntityId.playerA, EntityId.playerARobot, EntityId.playerBRobot),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(position(2, 2), ShapeType.robot).save()
            .buildEntity(EntityId.playerBRobot).withPhysicalComponent(position(3, 2), ShapeType.robot).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.match, [EntityId.match]], [EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]]])).save()
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerARobot, Physical, new Physical(EntityId.playerARobot, position(2, 2), ShapeType.robot)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBRobot, Physical, new Physical(EntityId.playerBRobot, position(3, 2), ShapeType.robot)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerARobot, EntityId.playerBRobot)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerARobot, EntityId.playerBRobot))
        ])
    serverScenario(`${Action.attack} 4 - Attack Game Event - playerB Robot`, attackEvent(EntityId.playerB, EntityId.playerBRobot, EntityId.playerARobot),
        ...
        , [
            ...
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerBRobot, EntityId.playerARobot))
        ])
    serverScenario(`${Action.attack} 5 - Can't Attack : Bad Phase for tower player A`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(3, 2), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsNotSent(TestStep.Then, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower)),
    ...
    serverScenario(`${Action.attack} 7 - Can't Attack : Out of Range - Horizontal 1`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(2, 2), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(24, 2), ShapeType.tower).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.match, [EntityId.match]], [EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]]])).save()
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(24, 2), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsNotSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', notifyEvent(EntityId.playerA, outOfRangeNotificationMessage))
        ])
    serverScenario(`${Action.attack} 8 - Can't Attack : Out of Range - Horizontal 2`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(24, 1), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(1, 1), ShapeType.tower).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.match, [EntityId.match]], [EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]]])).save()
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(24, 1), ShapeType.tower)),
    ...
    serverScenario(`${Action.attack} 8 - Can Attack : On Range - Horizontal 3 - Max Range`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(11, 1), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(1, 1), ShapeType.tower).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.match, [EntityId.match]], [EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]]])).save()
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(11, 1), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(1, 1), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower))
        ])
    serverScenario(`${Action.attack} 9 - Can't Attack : Out of Range - Vertical 1`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(position(1, 1), ShapeType.tower).save()
            .buildEntity(EntityId.playerBTower).withPhysicalComponent(position(1, 24), ShapeType.tower).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.match, [EntityId.match]], [EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]]])).save()
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(1, 24), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsNotSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', notifyEvent(EntityId.playerA, outOfRangeNotificationMessage))
        ])
    serverScenario(`${Action.attack} 10 - Can't Attack : Out of Range - Vertical 2`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        ...
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(1, 3), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(24, 3), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsNotSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', notifyEvent(EntityId.playerA, outOfRangeNotificationMessage))
        ])
    serverScenario(`${Action.attack} 11 - Can Attack : On Range - Vertical 3 - Max Range`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        ...
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(1, 1), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(1, 11), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower))
        ])
    serverScenario(`${Action.attack} 12 - Can't Attack : Out of Range - Diagonal`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        ...
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(24, 24), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsNotSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', notifyEvent(EntityId.playerA, outOfRangeNotificationMessage))
        ])
    serverScenario(`${Action.attack} 13 - Can Attack : On Range - Diagonal - Max Range`, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower),
        ...
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 1), ShapeType.tower)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(11, 4), ShapeType.tower)),
            (game, adapters) => whenEventOccurs(game, attackEvent(EntityId.playerA, EntityId.playerATower, EntityId.playerBTower)),
            (game, adapters) => theEventIsSent(TestStep.And, adapters, 'server', hitEvent(EntityId.playerATower, EntityId.playerBTower))
        ])
