...
describe(featureEventDescription(Action.quit), () => {
    serverScenario(`${Action.quit} 1`, quitMatchEvent(matchId, playerAId),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(matchId).withPlayers([playerAId, playerBId]).save()
            .buildEntity(playerAId).withEntityReferences(new Map([[EntityType.robot, [playerARobotId]], [EntityType.tower, [playerATowerId]]])).save()
        , [
    ...
    serverScenario(`${Action.quit} 2`, quitMatchEvent(matchId, playerBId),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(matchId).withPlayers([playerBId]).save()
            .buildEntity(playerBId).withEntityReferences(new Map([[EntityType.robot, [playerBRobotId]], [EntityType.tower, [playerBTowerId]]])).save()
        , [
