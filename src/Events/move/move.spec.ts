...
feature(featureEventDescription(Action.move), () => {
    serverScenario(`${Action.move} 1 - Move Game Event - Horizontal`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerATower).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.tower, true).save()
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(playerARobotFirstPosition(), ShapeType.robot, true).save()
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
            ...
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 1), ShapeType.tower, true)),
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerA, new Physical(EntityId.playerATower, position(2, 1), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerB, new Physical(EntityId.playerATower, position(2, 1), ShapeType.tower, true))
            ])
        ])
    serverScenario(`${Action.move} 2 - Move Game Event - Vertical`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx1y2),
        ...
        , [
            ...
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(1, 2), ShapeType.tower, true)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase(11))),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerA, new Physical(EntityId.playerATower, position(1, 2), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerB, new Physical(EntityId.playerATower, position(1, 2), ShapeType.tower, true))
            ])
        ])
    serverScenario(`${Action.move} 3 - Move Game Event - Diagonal`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerATower).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.tower, true).save()
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(position(1, 2), ShapeType.robot, true).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
        , [
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Given, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase())),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
            ...
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower, true)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.match, Phasing, new Phasing(EntityId.match, playerATowerPhase(11))),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerA, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerB, new Physical(EntityId.playerATower, position(2, 2), ShapeType.tower, true))
            ])
        ])
    serverScenario(`${Action.move} 4 - Can't Move Game Event - Tower already on destination cell`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.cellx2y2).withPhysicalComponent(position(2, 2), ShapeType.cell, true).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.tower, true).save()
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.robot, true).save()
        ...
        , [
            ...
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [notifyEvent(EntityId.playerA, positionAlreadyOccupiedNotificationMessage)])
        ])
    serverScenario(`${Action.move} 5 - Can't Move Game Event - Tower of same player already on destination cell`, moveEvent(EntityId.playerB, EntityType.robot, EntityId.playerBRobot, EntityId.cellx2y2),
        ...
        , [
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [notifyEvent(EntityId.playerB, positionAlreadyOccupiedNotificationMessage)])
        ])
    serverScenario(`${Action.move} 6 - Can't Move Game Event - Robot of same player already on destination cell`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerBTower, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerBTowerPhase()).withPlayers([EntityId.playerA, EntityId.playerB]).save()
            .buildEntity(EntityId.playerBRobot).withPhysicalComponent(position(2, 2), ShapeType.robot, true).save()
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerBRobot, Physical, new Physical(EntityId.playerBRobot, position(2, 2), ShapeType.robot, true)),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.cellx2y2, Physical, new Physical(EntityId.cellx2y2, position(2, 2), ShapeType.cell, true)),
            ...
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, position(1, 1), ShapeType.tower, true)),
    ...
    serverScenario(`${Action.move} 7 - Can't Move Game Event - No action points`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildEntity(EntityId.playerATower).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.tower, true).save()
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(playerATowerFirstPosition(), ShapeType.tower, true).save()
            .buildEntity(EntityId.cellx2y1).withPhysicalComponent(position(2, 1), ShapeType.cell, true).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
        , [
    ...
    serverScenario(`${Action.move} 8 - Can't Move Game Event - Bad Player`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            ...
            .buildTower(EntityId.playerBTower, playerBTowerFirstPosition(gridDimension)).save()
    ...
    serverScenario(`${Action.move} 10 - Auto move on player A Tower Placement Phase`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx1y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerAutoPlacementPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerA, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerB, new Physical(EntityId.playerATower, playerATowerFirstPosition(), ShapeType.tower, true)),
    ...
    serverScenario(`${Action.move} 11 - Auto move on player A Robot Placement Phase`, moveEvent(EntityId.playerA, EntityType.robot, EntityId.playerARobot, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerARobotAutoPlacementPhase()).withPlayers([EntityId.playerA]).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
            .buildTower(EntityId.playerATower, position(0, 0)).save()
            .buildRobot(EntityId.playerARobot, position(0, 0)).save()
        ...
        , [
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerARobot, Physical, new Physical(EntityId.playerARobot, playerARobotFirstPosition(), ShapeType.robot, true)),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.robot, EntityId.playerARobot, EntityId.playerA, new Physical(EntityId.playerARobot, playerARobotFirstPosition(), ShapeType.robot, true)),
                drawEvent(EntityType.robot, EntityId.playerARobot, EntityId.playerB, new Physical(EntityId.playerARobot, playerARobotFirstPosition(), ShapeType.robot, true)),
                nextTurnEvent(EntityId.match)
            ])
        ])
    serverScenario(`${Action.move} 12 - Auto move on player B Tower Placement Phase`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerBTower, EntityId.cellx24y24),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerBTowerAutoPlacementPhase()).withPlayers([EntityId.playerA, EntityId.playerB]).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
            .buildEntity(EntityId.playerB).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerBTower]], [EntityType.robot, [EntityId.playerBRobot]], [EntityType.match, [EntityId.match]]])).save()
            .buildTower(EntityId.playerATower, position(0, 0)).save()
            .buildRobot(EntityId.playerARobot, position(0, 0)).save()
            ...
            .buildEntity(EntityId.cellx24y24).withPhysicalComponent(playerBTowerFirstPosition(gridDimension), ShapeType.cell, true).save()
        , [
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerBTower, Physical, new Physical(EntityId.playerBTower, playerBTowerFirstPosition(gridDimension), ShapeType.tower, true)),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.tower, EntityId.playerBTower, EntityId.playerA, new Physical(EntityId.playerATower, playerBTowerFirstPosition(gridDimension), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerBTower, EntityId.playerB, new Physical(EntityId.playerATower, playerBTowerFirstPosition(gridDimension), ShapeType.tower, true)),
                nextTurnEvent(EntityId.match)
            ])
        ])
    serverScenario(`${Action.move} 13 - Auto move on player B Robot Placement Phase`, moveEvent(EntityId.playerB, EntityType.robot, EntityId.playerBRobot, EntityId.cellx24y24),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerBRobotAutoPlacementPhase()).withPlayers([EntityId.playerA, EntityId.playerB]).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
            .buildEntity(EntityId.playerB).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerBTower]], [EntityType.robot, [EntityId.playerBRobot]], [EntityType.match, [EntityId.match]]])).save()
            .buildTower(EntityId.playerATower, position(0, 0)).save()
            .buildRobot(EntityId.playerARobot, position(0, 0)).save()
            .buildTower(EntityId.playerBTower, position(0, 0)).save()
            .buildRobot(EntityId.playerBRobot, position(0, 0)).save()
            .buildEntity(EntityId.cellx24y24).withPhysicalComponent(playerBRobotFirstPosition(gridDimension), ShapeType.cell, true).save()
        , [
            ...whenEventOccured(),
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerBRobot, Physical, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridDimension), ShapeType.robot, true)),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                drawEvent(EntityType.robot, EntityId.playerBRobot, EntityId.playerA, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridDimension), ShapeType.robot, true)),
                drawEvent(EntityType.robot, EntityId.playerBRobot, EntityId.playerB, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridDimension), ShapeType.robot, true)),
                nextTurnEvent(EntityId.match)
            ])
        ])
})
