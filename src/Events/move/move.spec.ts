...
feature(featureEventDescription(Action.move), () => {
    serverScenario(`${Action.move} 1 - Move Game Event - Horizontal`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.And, adapters, EntityId.playerATower, Physical, new Physical(EntityId.playerATower, playerATowerFirstPosition(gridFirstCellPosition), ShapeType.tower, true)),
    ...
    serverScenario(`${Action.move} 4 - Can't Move Game Event - Tower already on destination cell`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withEntityReferences(EntityType.match, new Map([[EntityType.player, [EntityId.playerA, EntityId.playerB]]])).save()
            .buildEntity(EntityId.cellx2y2).withPhysicalComponent(position(2, 2), ShapeType.cell, true).save()
            ...
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(playerATowerFirstPosition(gridFirstCellPosition), ShapeType.robot, true).save()
    ...
    serverScenario(`${Action.move} 6 - Can't Move Game Event - Robot of same player already on destination cell`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerBTower, EntityId.cellx2y2),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerBTowerPhase()).withEntityReferences(EntityType.match, new Map([[EntityType.player, [EntityId.playerA, EntityId.playerB]]])).save()
    ...
    serverScenario(`${Action.move} 7 - Can't Move Game Event - No action points`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase(0)).withEntityReferences(EntityType.match, new Map([[EntityType.player, [EntityId.playerA]]])).save()
            .buildEntity(EntityId.playerATower).withPhysicalComponent(playerATowerFirstPosition(gridFirstCellPosition), ShapeType.tower, true).save()
            .buildEntity(EntityId.playerARobot).withPhysicalComponent(playerATowerFirstPosition(gridFirstCellPosition), ShapeType.tower, true).save()
            .buildEntity(EntityId.cellx2y1).withPhysicalComponent(position(2, 1), ShapeType.cell, true).save()
            .buildEntity(EntityId.playerA).withEntityReferences(EntityType.player, new Map([[EntityType.tower, [EntityId.playerATower]], [EntityType.robot, [EntityId.playerARobot]], [EntityType.match, [EntityId.match]]])).save()
        , [
    ...
    serverScenario(`${Action.move} 8 - Can't Move Game Event - Bad Player`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerATower, EntityId.cellx2y1),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
            .buildEntity(EntityId.match).withPhase(playerATowerPhase()).withEntityReferences(EntityType.match, new Map([[EntityType.player, [EntityId.playerA, EntityId.playerB]]])).save()
            .buildTower(EntityId.playerATower, playerATowerFirstPosition(gridFirstCellPosition)).save()
    ...
    serverScenario(`${Action.move} 10 - Auto move on player A Tower Placement Phase`, moveEvent(EntityId.playerA, EntityType.tower, EntityId.playerATower, EntityId.cellx1y1),
        ...
        , [
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                ...
                drawEvent(EntityType.tower, EntityId.playerATower, EntityId.playerA, new Physical(EntityId.playerATower, playerATowerFirstPosition(gridFirstCellPosition), ShapeType.tower, true))
            ])
        ])
    serverScenario(`${Action.move} 11 - Auto move on player A Robot Placement Phase`, moveEvent(EntityId.playerA, EntityType.robot, EntityId.playerARobot, EntityId.cellx2y2),
        ...
        , [
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                nextTurnEvent(EntityId.match),
                drawEvent(EntityType.robot, EntityId.playerARobot, EntityId.playerA, new Physical(EntityId.playerARobot, playerARobotFirstPosition(gridFirstCellPosition), ShapeType.robot, true))
            ])
        ])
    serverScenario(`${Action.move} 12 - Auto move on player B Tower Placement Phase`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerBTower, EntityId.cellx24y24),
        ...
        , [
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                nextTurnEvent(EntityId.match),
                drawEvent(EntityType.tower, EntityId.playerBTower, EntityId.playerA, new Physical(EntityId.playerBTower, playerBTowerFirstPosition(gridFirstCellPosition, gameScreenDimension), ShapeType.tower, true)),
                drawEvent(EntityType.tower, EntityId.playerBTower, EntityId.playerB, new Physical(EntityId.playerBTower, playerBTowerFirstPosition(gridFirstCellPosition, gameScreenDimension), ShapeType.tower, true))
            ])
        ])
    serverScenario(`${Action.move} 13 - Auto move on player B Robot Placement Phase`, moveEvent(EntityId.playerB, EntityType.robot, EntityId.playerBRobot, EntityId.cellx24y24),
        ...
        , [
            ...
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                nextTurnEvent(EntityId.match),
                drawEvent(EntityType.robot, EntityId.playerBRobot, EntityId.playerA, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridFirstCellPosition, gameScreenDimension), ShapeType.robot, true)),
                drawEvent(EntityType.robot, EntityId.playerBRobot, EntityId.playerB, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridFirstCellPosition, gameScreenDimension), ShapeType.robot, true))
            ])
        ])
    serverScenario(`${Action.move} 14 - Tower can't move.`, moveEvent(EntityId.playerB, EntityType.tower, EntityId.playerBTower, EntityId.cellx24y24),
        (game, adapters) => () => new EntityBuilder(adapters.entityInteractor)
        ...
        , [
            ...
            (game, adapters) => theEntityWithIdHasTheExpectedComponent(TestStep.Then, adapters, EntityId.playerBRobot, Physical, new Physical(EntityId.playerBRobot, playerBRobotFirstPosition(gridFirstCellPosition, gameScreenDimension), ShapeType.robot, true)),
            (game, adapters) => eventsAreSent(TestStep.And, adapters, 'server', [
                notifyPlayerEvent(EntityId.playerB, "Tower can't move.")
            ])
        ], undefined, true)
})
